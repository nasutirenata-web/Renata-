...
export default function LinkedInPage() {
  return <><PageHeader title="LinkedIn" description="Tu espacio para preparar contenido, diseñar piezas y organizar la presencia de tu marca."/><div className="space-y-6 p-5 sm:p-8">
    ...
    ].map(({icon:Icon,...a})=><Card key={a.title} className="flex flex-col"><Icon className="h-7 w-7 text-brand"/><CardTitle className="mt-5">{a.title}</CardTitle><CardDescription className="mb-6 mt-3">{a.body}</CardDescription><LinkButton href={a.href} className="mt-auto w-fit">Abrir <ArrowUpRight className="h-4 w-4"/></LinkButton></Card>)}</div>
